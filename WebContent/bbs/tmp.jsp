...
<body>
	...
	<%
		...
		BbsVO bbs = (BbsVO) request.getAttribute("rs");
	%>
	
	<table class="view_table" align=center>
        <tr>
                <td colspan="4" class="view_title"><%= bbs.getBbsTitle().replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
        </tr>
        <tr>
...
<div class="reply_view">
	<h3>댓글목록</h3>
		<%
			...
			int repCount = rep.getCount(bbsIdx);
			ArrayList<RepVO> list = rep.getList(bbsIdx);
...
<!--- 댓글 입력 폼 -->
	...
	<div class="co_form">
	<form method="post" class="reply_form" action="repWrite.re">
			<input type="text" name="bbsIdx" class="bno" value="<%= bbs.getBbsIdx() %>">
			<input type="text" name="userID" id="dat_user" class="dat_user" value="<%= session.getAttribute("userID") %>">
			<input type="text" name="userName" id="dat_user" class="dat_user" value="<%= session.getAttribute("userName")  %>">
			<div>
				<textarea name="repContent" class="reply_content" id="re_content" ></textarea>
	 ...
	 <%-- <%@ include file="/bbs/reply.jsp" %> --%>
 <h3>총 게시글 개수 <%= repCount %></h3>
</body>
</html>